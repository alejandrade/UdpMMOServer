...
public class ConnectReceiver implements MessageReceiver<ConnectMessageRequest, EmptyResponse> {
    ...
    public void handle(final String requestId,
        ...
        if (publisherHelper.isConnected()) {
            publisherHelper.respondSender(new BaseResponse<PayloadMessageResponse>(
                    ResponseMessageType.success, new EmptyResponse()));
        } else {
